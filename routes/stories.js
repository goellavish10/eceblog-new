...
router.post("/search", async (req, res) => {
  ...
  try {
    ...
    res.render("stories/index", {
      ...
      headOfPage: `Search results for '${fltrTitle}'`,
    });
  } catch (err) {
    console.log(err);
    res.redirect("/");
...
router.get("/:id/comments", ensureAuth, async (req, res) => {
  try {
    ...
    res.render("stories/readComments", {
      ...
      comments,
      idOfPost,
      headOfPage: `Comments on the post ${post.title}`,
    });
  } catch (err) {
    console.error(err);
    res.render("error/404", {
  ...
  try {
    ...
    if (commentorName != req.user.displayName) {
      ...
      await Story.updateOne(
        ...
        {
          $pull: {
            comments: { username: commentorName, comment: commentValue },
          },
        },
        {},
        (err, effect) => {
